...
class SuperKitSlider extends StatefulWidget {
  @override
  final Key? key;
  final Axis axis;
...
class _SuperKitSliderState extends State<SuperKitSlider>
  ...
  double? _widgetMin;
  double? _widgetMax;
  List<FlutterSliderIgnoreSteps> _ignoreSteps = <FlutterSliderIgnoreSteps>[];
  final List<FlutterSliderFixedValue> _fixedValues =
      <FlutterSliderFixedValue>[];

  List<Positioned> _points = <Positioned>[];
  ...
  @override
  void didUpdateWidget(SuperKitSlider oldWidget) {
    __isInitCall = false;
    initMethod();
...
//        left = (p * distance) + _handlersPadding - labelBoxHalfSize - 0.5;
    ...
    if (hatchMark.labels != null && hatchMark.labels!.isNotEmpty) {
      List<Widget> labelWidget = <Widget>[];
      Widget? label;
      ...
      for (FlutterSliderHatchMarkLabel markLabel in hatchMark.labels!) {
        ...
        if (widget.axis == Axis.horizontal) {
          ...
          labelBoxHalfSize = hatchMark.labelBox!.height / 2;
        }
...
//          left = (tr * distance) + _handlersPadding - labelBoxHalfSize - 0.5;
  ...
  void _setParameters() {
    ...
    _ignoreSteps = <FlutterSliderIgnoreSteps>[];
    ...
    if (widget.fixedValues != null && widget.fixedValues!.isNotEmpty) {
      ...
      List<double> fixedValuesIndices = <double>[];
...
//    items      ..addAll(_points);
  ...
  _trackBarSlideCallDragStated(dynamic handlerIndex) {
...
///////////////////////////////////////------- heho
  Positioned _activeTrack() {
    ...
    //Color trackBarColor = boxDecoration.color ?? Theme.of(context).primaryColor;
    if (widget.disabled) {
